...
function getNearZeroRefillConfig(
  metric: 'waterLevel' | 'phDown' | 'phUp',
  seed: number,
): {
  nearZeroMax: number;
  undershoot: number;
  ...
  if (metric === 'waterLevel') {
    return {
      nearZeroMax: 4 + pseudoRandom(seed, 12) * 4,
      undershoot: 1 + pseudoRandom(seed, 13) * 2,
      refillBoost: 28 + pseudoRandom(seed, 11) * 20,
      decayPerStep: 2.8 + pseudoRandom(seed, 10) * 2.2,
      max: 100,
    };
  }

  return {
    nearZeroMax: 0.35 + pseudoRandom(seed, 12) * 0.4,
    undershoot: 0.06 + pseudoRandom(seed, 13) * 0.08,
    refillBoost: 1.8 + pseudoRandom(seed, 11) * 1.4,
    decayPerStep: 0.22 + pseudoRandom(seed, 10) * 0.16,
    max: METRIC_BOUNDS[metric].max,
  };
}

function buildRefillableDecayHistory(
  metric: 'waterLevel' | 'phDown' | 'phUp',
  currentValue: number,
  machineId: string,
  hours: HistoryHours,
  ...
  const config = getNearZeroRefillConfig(metric, seed);

  const startValue = clamp(
    safeCurrent +
      config.decayPerStep * (pointCount - 1) * 0.62 +
      refillIndices.length * config.refillBoost * 0.28,
    safeCurrent + (metric === 'waterLevel' ? 4 : 0.25),
    config.max,
  );
  ...
  for (let i = 0; i < pointCount; i += 1) {
    const noiseScale = metric === 'waterLevel' ? 1.4 : 0.05;
    const noise = (pseudoRandom(seed, i + 1) - 0.5) * noiseScale;
    let value: number;
    ...
    if (i === 0) {
      ...
      value =
        Math.min(
          values[i - 1] - config.decayPerStep * 1.4,
          config.nearZeroMax - config.undershoot,
        ) + noise * 0.35;
    } else if (refillIndices.includes(i - 1)) {
      value = Math.min(values[i - 1] + config.refillBoost + noise, config.max);
    } else {
      const nextRefill = refillIndices.find((index) => index > i);
      value = values[i - 1] - config.decayPerStep + noise;
      if (nextRefill !== undefined && nextRefill - i <= 2) {
        value -= config.decayPerStep * 0.6;
      }
    }
    ...
    value = clamp(roundMetric(metric, value), 0, config.max);
...
function buildWaterMetricHistory(
  ...
  return buildRefillableDecayHistory('waterLevel', currentValue, machineId, hours);
}
...
function buildMonotonicDecayHistory(
  ...
  const now = Date.now();
  const pointCount = historyPointCount(hours);
  const decayPerStep = 2.2 + pseudoRandom(seed, 10) * 1.8;
  ...
  for (let i = 0; i < pointCount; i += 1) {
    const noise = (pseudoRandom(seed, i + 1) - 0.65) * 0.8;
    let value: number;
...
export function buildMetricHistory(
  ...
  if (metric === 'phDown' || metric === 'phUp') {
    return buildRefillableDecayHistory(metric, currentValue, machineId, hours);
  }
...
export type PumpHistoryKind = 'add' | 'ph-up' | 'ph-down';

export type PumpHistoryEntry = {
  id: string;
  at: Date;
...
function pumpAddLabel(metricKey: MetricKey): string {
  if (metricKey === 'waterLevel') return 'Water +';
  if (metricKey === 'phUp') return 'pH up +';
...
export function extractPumpHistory(
  ...
  const entries: PumpHistoryEntry[] = [];

  history.forEach((point, index) => {
    if (point.nutrientDose) {
      entries.push({
        id: `add-${index}`,
        at: point.at,
        kind: 'add',
